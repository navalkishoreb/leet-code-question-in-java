...
public class CombinationSum2 {
    class Solution {
        public List<List<Integer>> combinationSum2(int[] candidates, int target) {
            ...
            // sort the input
            // so that we remove duplicates
            // how duplicate entries are cnsidered in solution ?
            // same level → skip
            // different level → allowed
            // for different level current array hold the path
            Arrays.sort(candidates);
            dfs(candidates, target, new ArrayList<>(), 0, result);
            return result;
        }

        private void dfs(int[] nums, int target, List<Integer> current, int start, List<List<Integer>> res) {
            if (target == 0) {
                // use copy
                // this is create immutable list
            ...
            // send start is required once
            // we backtrack we don't want to repeat indexes
            for (int i = start; i < nums.length; i++) {

                // skip the duplicate entries
                // should be compared from provided start
                if (i > start && nums[i - 1] == nums[i]) continue;

                current.add(nums[i]);
                // re-use the same index is not allowed
                dfs(nums, target - nums[i], current, i + 1, res);
                current.removeLast();
            }
        }
