...
public class SlidingWindowMaximum {
    class Solution {
        public int[] maxSlidingWindow(int[] nums, int k) {
            ...
            // we need to remove element from front to slide window --> queue LIFO
            // but we need to compare current element with history
            // and remove the element which are lesser than current from history
            // stack operation LIFO
            // so we need both characteritics hence Dqe
            ...
            for (int i = 0; i < n; i++) {
                ...
                // remove the elements which are lesser than current value
                while (!window.isEmpty() && nums[window.peekLast()] < nums[i]) {
                    window.pollLast();
                }
                // add current index/ element in window
                window.offerLast(i);
                if (i - k + 1 >= 0) {
